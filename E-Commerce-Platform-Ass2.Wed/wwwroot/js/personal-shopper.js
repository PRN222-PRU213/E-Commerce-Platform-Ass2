(function () {
  ...
  async function sendMessage() {
    ...
    try {
      ...
      appendMessage(
        "⚠️ " +
          (err.message || "Đã xảy ra lỗi khi kết nối AI. Vui lòng thử lại."),
        "assistant",
      );
    } finally {
      isLoading = false;
  ...
  function showTyping() {
    ...
    div.innerHTML =
      '<div class="shopper-bubble shopper-typing">AI đang suy nghĩ...</div>';
    messagesEl.appendChild(div);
    messagesEl.scrollTop = messagesEl.scrollHeight;
  }
  ...
  function renderCombos(combos) {
    ...
    header.style.cssText =
      "padding:10px 2px 6px; font-weight:600; color:#6f42c1; font-size:0.875rem;";
    ...
    combos.forEach((combo, idx) => {
      ...
      (combo.products || []).forEach((p) => {
        ...
        const imgSrc =
          p.imageUrl || "https://via.placeholder.com/44x44?text=SP";
        item.innerHTML = `
  ...
  async function handleAddCombo(combo, btn) {
    ...
    const variantIds = (combo.products || [])
      .map((p) => p.variantId)
      .filter((id) => id);
    ...
    try {
      ...
      if (!res.ok) {
        const errorMessage = body?.error || "Khong the them vao gio hang.";
        throw new Error(errorMessage);
      }

      const addedCount =
        body?.count ?? body?.result?.addedCount ?? variantIds.length;
      const cartItemCount = body?.cartItemCount;

      if (Number.isFinite(Number(cartItemCount))) {
        if (window.realTime?.updateCartCountBadge) {
          window.realTime.updateCartCountBadge(cartItemCount);
        }
        document.dispatchEvent(
          new CustomEvent("rt:cart-count-updated", {
            detail: { count: Number(cartItemCount) },
            bubbles: true,
          }),
        );
      }
      ...
      appendMessage(
        `✅ Đã thêm bộ "${combo.name}" (${addedCount} sản phẩm) vào giỏ hàng!`,
        "assistant",
      );
    } catch (err) {
      btn.disabled = false;
      ...
      appendMessage(
        `⚠️ ${err?.message || "Không thể thêm vào giỏ hàng. Vui lòng thử lại."}`,
        "assistant",
      );
    }
  }
