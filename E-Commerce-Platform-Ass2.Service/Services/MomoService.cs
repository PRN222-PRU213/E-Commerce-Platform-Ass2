...
namespace E_Commerce_Platform_Ass2.Service.Services
{
    public class MomoService : IMomoService
    {
        ...
        /// </summary>
        public async Task<string> CreateTopUpPaymentAsync(long amount, string orderInfo)
        {
            var orderId = Guid.NewGuid().ToString();
            var requestId = Guid.NewGuid().ToString();

            // Sử dụng callback URL riêng cho TopUp
            var topUpReturnUrl = _config.ReturnUrl.Replace("/Checkout/PaymentCallBack", "/Wallet/TopUpCallback");
            var topUpNotifyUrl = _config.NotifyUrl; // Có thể giữ nguyên hoặc thay đổi

            string rawHash =
                $"accessKey={_config.AccessKey}" +
                $"&amount={amount}" +
                $"&extraData=" +
                $"&ipnUrl={topUpNotifyUrl}" +
                $"&orderId={orderId}" +
                $"&orderInfo={orderInfo}" +
                $"&partnerCode={_config.PartnerCode}" +
                $"&redirectUrl={topUpReturnUrl}" +
                $"&requestId={requestId}" +
                $"&requestType={_config.RequestType}";
